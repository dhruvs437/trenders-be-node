...
router.post('/api/addToCart', auth, async (req,res)=>{
    try{
        const {productId,add,qty,type} = req.body;
        if(add){
            let data;
            if(type=='add'){
                ...
                data = await User.findOneAndUpdate({_id:req.user._id},{$push:{cart:details}});
            }else{
                data = await User.findOneAndUpdate({_id:req.user._id,'cart.productId':`${productId}`},{$set:{'cart.$.qty':qty}});
            }
            return res.status(200).json({data:data});
        }else{
            const data = await User.findOneAndUpdate({_id:req.user._id},{$pull:{cart:{productId:productId}}});
            return res.status(200).json({data:data});
        }
    }catch(err){
